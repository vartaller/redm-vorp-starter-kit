...
| `bln_playername` | имена игроков над головой (скомпилирован, показывает source ID — заменяется на `varta_playername`) |
...
## Незавершённые разработки

### varta_playername (ветка `res/player-name`)
Скрипт отображения `[charId] Имя Фамилия` над игроками при зажатии Alt.
Написан как замена `bln_playername` (тот скомпилирован и показывает source ID вместо char_id).
**Статус:** ресурс запускается без ошибок, но текст не отображается. Причина не установлена.
Возможные проблемы:
- `IsControlPressed(0, 0x8AAA0AD4)` — неизвестно работает ли в RDR3 с этим хэшем
- `SetDrawOrigin` + `DisplayText(str, 0, 0)` — не проверено работает ли в 3D режиме
- Данные персонажа приходят корректно (это проверено через серверные логи)

## Идеи на будущее

| Идея | Описание |
|------|----------|
| Спавн животных для охоты | Спавнить животных вокруг игрока в зависимости от региона карты, если `vorp_hunting` или аналог не покрывает это — чтобы охота была доступна везде без блужданий |
